...
@Component({
  selector: 'app-list-employee-sanc-rec',
  templateUrl: './list-employee-sanc-rec.component.html',
  styleUrl: './list-employee-sanc-rec.component.css'
})
export class ListEmployeeSancRecComponent implements OnInit{
  ...
  addSanction(userId: number): void {
    // Navigate to the add sanction page with the user id as a parameter
    this.router.navigate(['add-sanction', userId]);
  }

  addReward(userId: number): void {
    // Navigate to the add sanction page with the user id as a parameter
    this.router.navigate(['add-reward', userId]);
  }
  ...
  getNewUsers(): void {
    ...
    this.appService.getALLUsers().subscribe((data: User[]) => {
      ...
      this.users = this.users.filter(user => 
        user.role.some(role => role.roleName === 'User' || role.roleName === 'GRH')
      );
